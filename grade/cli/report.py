...
@click.option('--format', type=click.Choice({'json', 'markdown', 'gradescope'}), default='markdown', help='output format to use')
@click.option('--output', type=click.File('w'), default='-', help='optional file to write output to')
def report(format, output):
    ...
    if format == 'markdown':
        score, max_score = sum((t['score'] for t in grades['tests'])), sum((t['max_score'] for t in grades['tests']))
        output.write('\n\n'.join([
            f"# Grade Results",
